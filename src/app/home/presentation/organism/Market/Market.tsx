...
const Market = ({ id }: MarketProps) => {
  return (
    <section className="section-Rocket" id={id}>
      ...
      <div className="empowerment">El empoderamiento del inversor.💪 Tu decides cuando vender.🤑</div>

      <section className="grid-icon">
        {Markets.map((item) => (
          <ComponentMarket key={item.title2} title2={item.title2} body2={item.body2} icon={item.icon} />
        ))}
      </section>
    </section>
  );
